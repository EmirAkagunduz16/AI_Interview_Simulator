...
export class InterviewsController {
  ...
  async findAll(
    ...
    const result = await this.interviewsService.findByUserId(userId, { page, limit, status });
    return {
      interviews: result.interviews.map((i) => i.toJSON() as unknown as InterviewResponseDto),
      total: result.total,
      page: result.page,
  ...
  async getStats(@Headers('x-user-id') userId: string): Promise<InterviewStatsDto> {
  ...
  @ApiResponse({ status: 403, description: 'Cannot start interview' })
  async create(
    @Headers('x-user-id') userId: string,
  ...
  async submitAnswer(
    ...
    const interview = await this.interviewsService.submitAnswer(userId, id, dto);
    return interview.toJSON() as unknown as InterviewResponseDto;
  }
