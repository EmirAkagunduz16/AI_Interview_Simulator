...
const RecordingArea = ({ isRecording: externalIsRecording, onClick, onRecordingComplete }: RecordingAreaProps) => {
  ...
  return (
    <div className={`recording-area ${recording ? 'recording-area--active' : ''} ${hasRecorded ? 'recording-area--completed' : ''}`}>
      {permissionDenied && (
        <p className="recording-area__error">
          Mikrofon erisimi reddedildi. Lutfen tarayici ayarlarindan mikrofon iznini verin.
        </p>
      )}
      
      <button className="recording-area__btn" onClick={handleClick} type="button">
        {recording ? (
      ...
      <div className="recording-area__info">
        {recording ? (
          <>
            ...
            <span className="recording-area__duration">{formatDuration(recordingDuration)}</span>
          </>
        ) : hasRecorded ? (
