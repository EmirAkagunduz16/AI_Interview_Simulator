...
// useAIInterview has been removed — voice interviews now use VAPI
// This component is legacy dead code; keeping a stub to prevent build errors
const useAIInterview = (_opts: Record<string, unknown>) => ({
  isLoading: false,
  isSpeaking: false,
  isListening: false,
  transcript: "",
  speakText: async (_t: string) => {},
  startListening: () => {},
  stopListening: () => {},
  stopSpeaking: () => {},
  processAnswerAndContinue: async (_t: string) => {},
...
const AudioQuestion = () => {
  ...
  const { currentQuestionIndex, questions, timeRemaining, isTimerRunning } =
  ...
  const {
    ...
    onUserSpeechResult: (text: string) => {
    ...
    onError: (err: string) => {
      ...
      if (err.includes("quota") || err.includes("exceeded")) {
        friendlyError =
          "Ses servisi şimdilik kullanılamıyor. Tarayıcı sesi kullanılacak.";
      } else if (err.includes("API key")) {
        friendlyError =
          "Ses servisi yapılandırılmamış. Tarayıcı sesi kullanılacak.";
      }
  ...
  useEffect(() => {
    ...
    const initializeInterview = async () => {
      ...
      const questionText =
        currentQuestion?.text || "Kendinizi kısaca tanıtır mısınız?";
  ...
  const canProceedToNext =
    conversationRound >= MAX_CONVERSATION_ROUNDS ||
    ...
    <div className="audio-question">
      ...
      <div className="audio-question__content">
        ...
        <div className="audio-question__phase">
          ...
          <span className="audio-question__phase-text">
            {getPhaseMessage()}
          </span>
        </div>
        ...
        <div className="audio-question__ai-section">
          <div className="audio-question__ai-avatar">
            ...
            {isSpeaking && (
              <span className="audio-question__speaking-indicator" />
            )}
          </div>
          ...
          <div className="audio-question__ai-bubble">
            ...
            {isSpeaking && (
              <button
                className="audio-question__skip-btn"
        ...
        <div className="audio-question__user-section">
          ...
          {conversationRound > 0 && (
            <div className="audio-question__progress">
              <span>
                Soru-Cevap: {conversationRound}/{MAX_CONVERSATION_ROUNDS}
              </span>
            </div>
          )}
        </div>
          ...
          <div className="audio-question__history">
            ...
            <div className="audio-question__history-list">
              {conversationHistory.map((item, index) => (
                <div
                  key={index}
                  className={`audio-question__history-item ${item.role}`}
                >
                  <span className="audio-question__history-role">
                    {item.role === "ai" ? "AI" : "Siz"}:
                  </span>
                  <span className="audio-question__history-text">
                    {item.text}
                  </span>
                </div>
              ))}
      ...
      <InterviewFooter
        buttonText={
          canProceedToNext ? "Sonraki Bölüme Geç" : "Gönder ve Devam Et"
        }
